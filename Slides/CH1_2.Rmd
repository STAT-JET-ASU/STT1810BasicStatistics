...
These slides are intended to accompany Section 1.2 Measuring the Strength of Evidence in [*Introduction to Statistical Investigations, 2nd Edition*](http://www.isi-stats.com/isi/index2nd.html). 
...
* [Example 1.2 Rock-Paper-Scissors](https://stat-jet-asu.github.io/STT1810BasicStatistics/Slides/Example1_2.html)
...
One way to assess strength is to use a *p*-value. The **p** stands for *probability*---it is the probability of getting a purely random value of the statistic (e.g., from simulation) that is at least as extreme as the observed statistic (*in the direction of $H_a$*) when $H_0$ is true.
...
The estimated *p*-value from my simulation is 0.0490. The results are statistically significant&mdash;that is, unlikely to be due to chance alone. We will conclude $H_a$ instead. I have ESP!

| Magnitude               | Guideline for Inference                                                   |
|-------------------------|---------------------------------------------------------------------------|
| *p*-value > 0.10        | not much evidence; $H_0$ is plausible                                     |
| 0.05 < *p*-value ≤ 0.10 | moderate evidence against $H_0$                                           |
| 0.01 < *p*-value ≤ 0.05 | strong evidence against $H_0$ **&larr; my estimated p-value falls here!** |
...
From a probability point of view, we *did* reject the plausibility of the chance model, but the breadth of conclusions are limited by the nature of the experiment and our sample.

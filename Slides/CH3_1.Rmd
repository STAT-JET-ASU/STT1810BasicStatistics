...
Test different values of $\pi$ under the null hypothesis.
...
Reject Ho *or* fail to reject Ho using the *p*-value and the chosen level of significance (most commonly 0.05).
...
Researchers tested a dog named Marine to see whether or not she could detect colorectal cancer. She smelled breath in five bags from five different patients, only one of which had cancer. Marine chose correctly in 30 of 33 attempts ($\hat{p} = 0.909$). 
...
We can estimate by testing candidate values of $\pi$ on either side of our observed sample proportion $\hat{p} = 0.909$.
...
We are 95% confident that Marine's true probability of detecting cancer is between 0.761 and 0.974.

This is much stronger than simply stating $\pi > 0.20$ (as in $H_a$).

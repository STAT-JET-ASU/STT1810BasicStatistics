...
* A representation that is meant to emulate reality, but which is often smaller or simpler than the real process or object.
...
***Example 2:*** For the Monty Hall game, we *assumed* that the prize was equally likely to be placed behind any of the three doors. In a different version, we might assume unequal probabilities.
...
***Example 1:*** For the ESP example, the parameter is each person's innate chance of correctly identifying a card. If they do not have ESP, that chance is 0.2, versus *more than* 0.2 if they do have ESP. This probability is reflected in their *long-run* performance on the test---many, many more trials than 25. 

***Example 2:*** Across a large number of instances of the Monty Hall game, what is the probability a wins player if they switch? This is one scenario we investigated using our 1000+ simulations of the game using the applet to simulate playing the game. 

The *long-run* probability of winning the game if you switch is the parameter for the random process.

***Example 3:*** For a "fair" coin, the parameter might be the chance of getting a head when the coin is flipped, which is 50%.
...
***Example 2:*** Playing one trial of the Monty Hall game is a unit; the number of rounds we play is the sample size.
...
**<mark>Strength of evidence</mark> :** Compare your observed statistic to many simulated values of the same statistic that you generate using a chance model that fits the situation you are investigating. 

If we decide the observed statistic is unlikely to occur by chance alone, we can logically conclude that the observed data provide strong evidence against the *plausibility* (*believability* or *credibility*) of the chance model --- but *NOT* "proof". 

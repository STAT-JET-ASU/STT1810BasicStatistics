...
* Calculate and interpret a *p*-value, and state the strength of evidence it provides against a null hypothesis. [Section 1.2]
...
In the field of Statistics, a **<mark>test of significance<mark>** is a procedure for measuring the strength of evidence against our null hypothesis.

(1) Make a claim about the parameter of interest ($H_0$ and $H_a$)
(2) Gather and explore data
(3) Follow the **3S** strategy to 
    * calculate an observed ***Statistic*** from the gathered data
    * ***Simulate*** a null distribution (*scenario assuming $H_0$ is true*)
    * compare the ***statistic*** to the ***simulated*** null distribution to measure the ***Strength of evidence*** it provides *against* $H_0$.  
(4) Draw a conclusion about the plausibility $H_0$ and whether the test result is statistically significant---leading us to reject $H_0$. 


## Quantifying *Strength of Evidence*

Assuming $H_0$ is true ... our **<mark>*p*-value</mark>** is the probability of getting a purely random value of the statistic that is at least as extreme as the observed statistic we are assessing.

| Magnitude of the *p*-value  | Strength of evidence provided by the *p*-value               |
|-----------------------------|--------------------------------------------------------------|
...
Researchers across many of disciplines consider **<mark>*p*-value ≤ 0.05</mark>** to be the threshold where we can: (1) claim that our test results have ***<mark>statistical significance</mark>***; (2) declare the chance model to be implausible; and thus (3) reject $H_0$ and accept $H_a$ instead. 
...
In some situations you may want or need stronger evidence. For example, rejecting $H_0$ may have life or death consequences in a medical study, so the researchers might use *p*-value ≤ 0.01 (only *very strong evidence*) as their chosen level of significance.
...
Ronald Fisher wrote *Statistical Methods for Research Workers* in 1925. He chose 1 in 20 (5%) as his general cutoff value, because it typically corresponds to about two standard deviations from the expected value. (*Though there is some [disagreement](https://www.scientificamerican.com/article/the-significant-problem-of-p-values/)...*)
...
## The ISI [One Proportion](http://www.isi-stats.com/isi2nd/ISIapplets2021.html) Applet
...
* ***Statistic*** goes in the Count Samples box
...
If you enter a "Probability of heads" other than 0.5, the labels change to more general terms. Notice the statistical symbols.

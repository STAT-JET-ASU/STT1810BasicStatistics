...
**null distribution :** a distribution of simulated statistics that are a representation of what *could have* happened in the actual study if $H_0$ were true (we *assume* $H_0$ is true in the simulation)
...
Our *p*-value is the probability of getting a purely random value of a statistic that is at least as extreme (*in the $H_a$ direction*) as the observed statistic we are assessing---assuming $H_0$ is true.

| Magnitude               | Guideline for Inference                                           |
|-------------------------|-------------------------------------------------------------------|
| *p*-value > 0.10        | not much evidence against the null hypothesis                     |
...
Researchers across many of disciplines consider *p*-value ≤ 0.05 (i.e., ≤ 5% chance) to be the threshold where we can claim data have ***statistical significance*** and declare the chance model ($H_0$) to be implausible. 

A *p*-value ≤ 0.05 indicates we have *strong* or *very strong evidence* against the null hypothesis in our textbook guidelines.

In some situations you may want or need stronger evidence. For example, rejecting $H_0$ may have life or death consequences in a medical study, so the researchers might use *p*-value ≤ 0.01 (*very strong evidence*) instead when assessing data.

<hr>

You may be wondering: why this particular probability? What's so special about it?


## Why 0.05?

Ronald Fisher wrote *Statistical Methods for Research Workers* in 1925 and he chose 1 in 20 (5%) as his general guide, because it typically corresponds to roughly two standard deviations from the expected value. (*Though not without some [controversy](https://www.scientificamerican.com/article/the-significant-problem-of-p-values/)...*)
...
For our ESP test using Zener cards, the parameter of interest is $\pi$ = the long-run probability that the person who is being tested can correctly identify the symbol on a card. Our two options are:

* the person is just randomly guessing and therefore $\pi$ = 0.20
* the person is not guessing and $\pi$ > 0.20 (*better than* guessing)

These two options lead to our null and alternative hypotheses, expressed below in statistical language.
...
**Statistic:** Say I get 9 of 25 cards correct (36%) on my ESP test. We usually use the success proportion as our statistic: $\hat{p} = 0.36$.

**Simulate:** We can simulate a null distribution for our Zener card ESP test using our [One Proportion](http://www.isi-stats.com/isi2nd/ISIapplets2021.html) applet, assuming the person is just guessing. We would set $\pi = 0.20$ and $n = 25$, with 1000 or more samples (repetitions). 

To calculate the *p*-value after simulating the null hypothesis, we will how many simulated samples that are $\ge \hat{p} = 0.36$, since that is the direction indicated in our alternative hypothesis.

In other words, if the actual statistic is unusually large compared to the simulated values that assume pure randomness, then it is evidence that I am *not* randomly guessing.
...
**Strength of Evidence:** From my simulation, I obtained a *p*-value of **0.0490**. My actual statistic is located somewhat far out in the upper (right) tail, as we can see on the simulation plot.

According to our guidelines, I have **strong evidence** against the null hypothesis. My results are **statistically significant** (unlikely to be due to just random chance). Therefore, I will reject $H_o$.
...
## What Does It Mean To Reject $H_o$?

TBA


## So remember our general rule...

<p>&nbsp;</p>

```{r, echo = FALSE, fig.align="center"}
knitr::include_graphics("../images/freddiemercurypvalue.jfif")
```

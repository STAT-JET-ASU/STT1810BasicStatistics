...
In this context, the **<mark>*z*-score</mark>** measures how extreme the observed statistic is relative to the value hypothesized in $H_0$. The $H_a$ will indicate whether we consider positive values (right tail) and / or negative values (left tail).
...
There are other versions of standardized scores with non-zero means that you might encounter. For example, IQ test scores are often adjusted to have a mean of 100 and SD of 15, similar to how 0 °C can be re-scaled to 32 °F. This is an example from the [Cora Lee Institute](https://coraleeinstitute.org/understanding-standardized-test-scores/).

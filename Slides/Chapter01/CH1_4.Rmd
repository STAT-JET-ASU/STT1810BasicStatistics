...
**Additional Learning Goal for Section 1.4**
...
***Example:*** In ESP testing, we can investigate "psi-hitting" (*getting more cards correct than expected by chance*) as well as an idea we did not address in P.1 called "psi-missing" (*too few cards correct*). To investigate both, we would write our hypotheses as follows.
...
* **<mark>extremeness</mark> :** The farther the observed statistic $\hat{p}$ is from the mean value of the null distribution (i.e., the value of $\pi$ given in $H_0$), the more evidence $\hat{p}$ typically provides against $H_0$. This is sometimes referred to as an *effect size*.
...
* **<mark>one- vs. two-sided $H_a$</mark> :** Because the *p*-value for a two-sided test is about twice as large as for a one-sided test using the same data, it provides less (weaker) evidence against $H_0$. Two-sided tests are used more often in scientific practice.
...
You decide to get tested for COVID-19 based on mild symptoms. There are two correct decisions and two errors that could occur (this is true of most medical tests).

...
Plug the mean and SD values from the Summary Statistics into their correct places, along with the statistic we computed from the data, $\hat{p} = 9/25 = 0.36$.
...
My *z*-score is exactly +2, so I have **strong evidence** against $H_0$.
...
Other $\hat{p}$ scores. Notice *z* values below, at, and above the mean*.
...
My result of 9 (36%) cards correct, with its *z*-score of 2, would be considered *statistically significant*. In other words, it is unlikely to be the result of just random chance. With such strong evidence, I would say the chance model is *implausible* and accept $H_a$. This finding agrees with my *p*-value results. 

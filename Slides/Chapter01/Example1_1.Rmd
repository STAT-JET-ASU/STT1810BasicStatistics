...
Button-pushing is the **random process** we are observing here.
...
Bastian used two light signals. (1) Steadily on meant “push the button on the right.” (2) Blinking meant “push the button on the left.” Correct pushes were rewarded with a fish. Doris and Buzz learned to choose the correct button every time.
...
Bastian put a curtain across the middle of the pool. Doris could see the light signal, but Buzz could not. When the light came on, Doris swam near the curtain and whistled. Buzz whistled back and pressed the correct button. Bastian repeated this process many times with both the steady light and the blinking light. He observed and recorded Buzz's button choice on each trial.

**parameter ($\pi$):** Buzz’s actual long-run proportion (or *probability*) of pushing the correct button based on whistles from Doris
...
To summarize these results, we report a numerical summary of the sample data that we call a  *statistic*. 
...
The proportion is $15/16 = 0.9375$. We could also say $93.75\%$. We use the symbol $\hat{p}$ to represent a sample proportion.
...
We will do $100$ simulations of the experiment, $16$ trials in each, assuming a purely random chance of correctly choosing ($0.5$).

```{r fig.align="center"}
knitr::include_graphics("../images/example01_01_100.png", dpi = 170)
```
...
The results of the simulation are displayed using a dotplot or a histogram so we can see the *null distribution* of the statistic.

```{r fig.align="center"}
knitr::include_graphics("../images/fig_01_01_04A.jpg", dpi = 110)
```

Any number of successes from $0$ to $16$ is *possible* if Buzz is just guessing. This dotplot of our simulation results shows us which of these possible values are more *probable* or *likely* to happen.


## 

When we talk about a long-term probability, $100$ simulations is not really very long-term. The textbook suggests at least 1000.

```{r fig.align="center"}
knitr::include_graphics("../images/example01_01_1000.png", dpi = 170)
```
...
```{r fig.align="center"}
knitr::include_graphics("../images/fig_01_01_04B.jpg", dpi = 85)
```
...
The plot shows the true long-run probabilities of getting $0$ to $16$ correct, if Buzz is guessing on each trial. Getting $8$ of $16$ ($50\%$) is the most common outcome, as we would predict, with numbers of successes becoming less likely as we go lower or higher.

```{r fig.align="center", fig.height = 3.2}
pmf <- tibble(
  successes = 0:16,
...
Most traditional statistical methods are *approximations* that use probability theory and mathematics. Section 1.5 (which we will not cover) presents a method that approximates probabilities using the Normal distribution ("bell curve"). You may have seen this method in AP Statistics or a research methods class.

Historically, we used these methods because of the challenges of calculating the values we needed without technology.
...
In any case, we can see that getting $15$ out of $16$ correct just by randomly guessing is a *very* unlikely, but not impossible.


## When is the Probability Small Enough?

We saw that the probability of Buzz getting $15$ of $16$ correct just by randomly guessing is very, very small---somewhere between $0.0002$ and $0.0003$. At that level, it is probably safe to say that most people would agree with the conclusion that Buzz was not just guessing. 

But what if the probability had not been quite so low? What if it was $1\%$? Or $4\%$? Or $12\%$?

Overall, we need a quantifiable and consistent way to make our statistical decisions. In upcoming sections, you will learn about *p*-values and *z*-scores, two methods for assessing the plausibility of "just randomness" as an explanation for the observed results of a research study.

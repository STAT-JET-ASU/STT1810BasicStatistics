...
Doing a larger number of simulations gives us a better idea of the long-run probability for all the possible outcomes. Getting a count of $5$ to $11$ correct just by guessing is fairly common. 


## If We Did Infinite Simulations...

The plot shows the true long-run probabilities of getting $0$ to $16$ correct, if Buzz is just guessing on each trial. Notice that $8$ ($50\%$ correct) is most common. *This requires doing a bit of math*...
...
Most traditional statistical methods are *approximations* that use probability theory and mathematics. We will see this in Section 1.5, where we will learn a method that approximates a solution using the Normal distribution.

In this course, we will use simulations to approximate long-run probabilities. In many ways, simulations can be preferable and more flexible than theory-based methods. You will be asked to compare simulations to theory-based results.

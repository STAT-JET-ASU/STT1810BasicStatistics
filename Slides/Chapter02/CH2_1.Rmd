...
Recall the terms *parameter*, *statistic*, and the other vocabulary we learned in Chapter 1 and the Preliminaries sections. Review as needed to make sure you have their meanings solidified, so that you can use statistical language effectively as we move on.
...
**<mark>simple random sample</mark> :** a subset of observational units chosen so that every sample of a given size $n$ is equally likely to be the sample that we select from the population (*each observational unit has the same chance of being chosen as every other unit*)

<hr> 

A sampling method is considered to be ***<mark>biased</mark>*** if the statistics calculated from different samples consistently *overestimate* or consistently *underestimate* the population parameter in which we are interested. *Non-random* samples tend to be biased.

<hr>

**<mark>nonsampling concerns</mark> :** potential sources of bias (e.g., poorly worded survey questions, people lying) that are not related to the way the sample was selected from the population
...
**<mark>sampling variability</mark> :** the amount that a statistic changes from sample to sample as we repeatedly observe it (*take a bunch of samples and calculate the same statistic---how different are they?*)
...
The ***<mark>Central Limit Theorem</mark>*** tells us that when we sample from a large finite population, the sampling distribution of the sample proportion $\hat{p}$, calculated from from repeated random samples, will be approximately normal if there are at least $10$ successes and at least $10$ failures in each sample.
...
The Central Limit Theorem describes the sampling distributions of certain statistics. Here, we are specifically interested in finite populations like most of those we encounter in real life, with our $\hat{p}$ statistic. We must meet some necessary criteria. 
...
* However, at the same time, we are taking random samples of size $n$ that are big enough that we have at least $10$ successes and $10$ failures in each sample.  
...
If the key CLT criteria on the previous slide are true for a given scenario, the sampling distribution of the sample proportion $\hat{p}$ will have the following distribution properties.
...
The standard deviation of the sampling distribution of $\hat{p}$ will be a function of $\pi$ and the sample size $n$.
...
&rarr; We call the SD of a sampling distribution its ***<mark>standard error</mark>***.


## Two Example Calculations

Plug in the appropriate values and solve. Notice that the smaller value of $\pi$ (given the same $n$) has a smaller SD.
...
Simulated sampling distribution using [One-Proportion](http://www.isi-stats.com/isi2nd/ISIapplets2021.html) applet:

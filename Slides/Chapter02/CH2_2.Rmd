...
The **<mark>Central Limit Theorem</mark>** describes the sampling distributions of certain statistics from finite populations, like $\bar{x}$, if we meet the validity criteria. Just like Section 2.1, we are sampling from large, finite populations.
...
**<mark>shape</mark> :** The sampling distribution will be approximately normal if the population is not too skewed and $n = 20$ or more. (*Many other sources will say $n = 30$ or more, to help account for skewed distributions and to be more cautious/conservative.*)
...
**<mark>center</mark> :** The mean of the sampling distribution will be equal to the population mean $\mu$ and $\bar{x}$ is unbiased like $\hat{p}$, so all possible values of the sample mean $\bar{x}$ for a given $n$ average out to be $\mu$.

...
**<mark>confounding variable</mark> :** a variable related to the explanatory and response variables in such a way that its effects on the response variable cannot be separated from the effects of the explanatory variable---it provides another possible "causal" explanation 
...
**<mark>experimental unit</mark> :** what observational units are often called in an experiment (also **subject** or **participant**)

**<mark>double blind</mark> :** a study design where neither the subjects nor the people who are evaluating the response know which treatment group each subject is in (*usually requires a placebo*)

**<mark>cause and effect</mark> :** if we do a randomized experiment, then we can conclude that the explanatory variable is causing the effect (difference or association) we see in the response variable (*the randomization balances possible confounding variables*)

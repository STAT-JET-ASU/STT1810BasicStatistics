...
We can calculate percentages of children with nearsightedness *conditional* on the amount of light in the room at night. In this case, each column (category of light) is treated as a separate group (the two proportions in each group will sum to 100%).
...
The observed probability of a child having having myopia is not only different for the three amounts of light, but there seems to be a pattern that connects having more light in a child's room to a greater probability of myopia.
...
Is there another possible explanation that you can think of for the effects that the researchers observed in the data, besides randomness and the amount of light in the room?

In other words, are there any *confounding variables* we need to look at for these data?
...
This is an observational study; we will revisit this term in Section 4.2. The researcher did not decide which children would sleep in which levels of light. Thus, even if we find an association, it does not prove that a ***cause and effect*** relationship exists. 
